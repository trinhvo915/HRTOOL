...
namespace Orient.Base.Net.Core.Api.Core.Business.Services
{
	...
	public class InterviewService : IInterviewService
	{
		...
		{
			var calendarType = await _calendarTypeRepository.FetchFirstAsync(x => x.Name == CalendarTypeConstants.Interview);
			...
			foreach (var userId in interviewManageModel.InterviewerIds)
			{
			...
			model.JobName = CalendarTypeConstants.Interview;
			...
			foreach (var user in listToUser)
			{
				...
				modelInterviewer.JobName = CalendarTypeConstants.Interview;
                ...
                {
                    emailTemplateResponseModelInterviewer.CC = string.Empty;
                    emailTemplateResponseModelInterviewer.BCC = string.Empty;
                }
		...
		public async Task<ResponseModel> DeleteInterviewAsync(Guid id)
		{
			...
			{
				await _attachmentInInterviewRepository.DeleteAsync(interview.AttachmentInInterviews);
				await _calendarRepository.DeleteAsync(interview.Calendar);
